...
check_token() {
  ...
  if [ "$USER_TOKEN" = "maklo666" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Satxzyon${NC}"
    echo -e "${YELLOW}TELEGRAM : @user001648292${NC}"
    echo -e "${YELLOW}WHATSAPP : +584168228057${NC}"
    echo -e "${YELLOW}© Satzxyon${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  ...
  THEME_URL="https://github.com/maklo666/Pterodactyl-Theme-Autoinstaller/raw/main/${THEME_NAME}.zip"
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/maklo666/Pterodactyl-Theme-Autoinstaller/raw/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Satzxyon   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : @user001648292     ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Satzxyon  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Satzxyon  ${NC}"
